...
def print_user_message(template_variables):
    print('I found ' + str(len(template_variables)) +
...
def render_lines_with(template_fillings, template_lines):
    ...
    for line in template_lines:
        matches = re.finditer(r'{{([^}]+)}}', line)
        for match in matches:
            current_filling = get_filling_for(match.group(1).strip(),
                                              match.group(0),
                                              template_fillings)
...
class VariableNotDefinedError(KeyError):
    def __init__(self, variable_representation):
        self.message = ('Variable ' + variable_representation +
                        ' was found, but not defined!')
